...
# v2.0 JS paths (replaces old Lua controller + HTM template)
...
echo "[2/4] Checking rpcd dependency..."
if ! command -v rpcd > /dev/null 2>&1; then
    echo "  rpcd: not found — installing..."
    if opkg install rpcd rpcd-mod-file; then
        echo "  rpcd: installed ✓"
    else
...
# Backend capture script (unchanged)
